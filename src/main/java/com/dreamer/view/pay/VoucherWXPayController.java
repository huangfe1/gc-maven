...
import com.dreamer.domain.user.VoucherTransferType;
...
import com.dreamer.service.pay.UnifiedOrderHandler;
	...
	public String pay(@RequestParam(required = false)String code,Integer orderId,Model model,HttpServletRequest request){
		try {
			...
			//if (Objects.isNull(agent.getWxOpenid()) || agent.getWxOpenid().isEmpty()) {//没有openid
				...
				HashMap<String, String> map = getOpenIdHandler.getOpenId(null, code);
			...
			String jsonParam = JsonUtil.mapToJsonStr(jsApiParameterFactory.build(null, unifiedOrder.getPrepay_id()));
