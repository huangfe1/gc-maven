                ...
                <div class="collapse navbar-collapse" id="transfer-navbar-collapse">
                    <form class="navbar-form navbar-left" id="searchForm"
                        ...
                        <div class="form-group">
                            <label>转入方</label>
                            <input type="text" name="entity.toAgent.realName" value="${parameter.entity.toAgent.realName}" class="form-control" id="applyAgent">
                            <label>转出方</label>
                            <input type="text" name="entity.FromAgent.realName" value="${parameter.entity.fromAgent.realName}" class="form-control" id="applyAgent">
                            <label>申请时间</label>
                            <div class="input-daterange input-group" id="datepicker">
                                <span class="input-group-addon"><span
                                        class="glyphicon glyphicon-calendar"></span></span>
                                <input type="text" class="form-control pointer" value="${parameter.startTime }" id="startDate" data-date-format="yyyy-mm-dd" name="startTime" placeholder="开始日期"/>
                                <span class="input-group-addon">到</span> <input type="text" class="form-control pointer" value="${parameter.endTime }" name="endTime" placeholder="截止日期"/>
                    ...
                    <table
                        ...
                        <c:forEach items="${trans}" var="g">
                            <tr data-row="${g.id}">
                                ...
                                <td>${g.toAgent.realName}(${g.toAgent.agentCode})</td>
                                <td>${g.fromAgent.realName}(${g.fromAgent.agentCode})</td>
                                <td>${g.quantity}</td>
                                <td>${g.amount}</td>
                                    <%--<td>${g.points}</td>--%>
                                <td>${g.applyTime}</td>
                                <td>${g.updateTime}</td>
                                <td>${g.voucher}+${g.purchase}</td>
                                <td>${g.status.desc}</td>
                                <td>${g.remittance}</td>
                                <td>${g.remark}</td>
                                <td>${g.operator}</td>
                                ...
                                <td>
                                    <a class="btn btn-info default detailBtn"
                                       href="<c:url value='/transfer/detail.html?id=${g.id}'/>">
                                        ...
                                        <c:if test="${g.fromAgent.id eq user.id  }">
                                        ...
                                        <c:if test="${g.toAgent.id eq user.id }">
                                    ...
                                    <%--</c:if>--%>
                                    <!--如果是退货-->
                                    <c:if test="${g.status eq 'BACK'}">
                                        <!--是管理员-->
                                        <c:if test="${user.admin }">
                                            <a class="btn btn-success default ajaxLink" href="<c:url value='/mobile/goods/confirmBackTransfer.json?tid=${g.id}' /> "><span class="fa fa-share-square-o" aria-hidden="true"></span>同意退货</a>
                                            <a class="btn btn-warning default ajaxLink" href="<c:url value='/mobile/goods/refuseBackTransfer.json?tid=${g.id}' /> "><span class="glyphicon glyphicon-minus-sign" aria-hidden="true"></span>拒绝退货</a>
                                        </c:if>
                                    </c:if>
                                </td>
