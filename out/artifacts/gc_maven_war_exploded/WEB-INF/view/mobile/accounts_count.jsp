...
<div action="<c:url value="/dmz/count.html"/>">
    ...
    <select name="addSub" id="addSub" class="form-control">
        <option value="-1">查询项目</option>
...
<script>
    ...
    $(function () {
        ...
        $(".sb").click(function () {
            ...
            if(addSub<0){
                alert("请选择查询项目");return;
            }
            search(uid,startTime,endTime,addSub);
        })
        ...
        function search(uid,startTime,endTime,addSub) {
            //首先清除上次的
            $(".btr").remove();
            var url = "<c:url value="/mobile/accounts/count.json"/>";
            var param = {
                "uid":uid,
                "startTime":startTime,
                "endTime":endTime,
...
<script id="bt" type="text/x-jquery-tmpl">
    <tr class="btr">
    <td class="btd">{{= param1}}</td>
    <td class="btd">{{= param2}}</td>
    </tr>
</script>
</body>
</html>
