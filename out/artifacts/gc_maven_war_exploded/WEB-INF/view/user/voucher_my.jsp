				...
				<div class="container-fluid">
					...
					<div class="col-sm-12 col-md-12 col-xs-12">
						<form class="navbar-form form-horizontal navbar-left" id="searchForm"
							role="search" action="<c:url value='/voucher/my.html'/>"
							method="GET">
							...
							<label>转让时间</label>
									...
									<label>接受人编号</label>
									<input type="text" name="toAgent" value="${parameter.entity.userByToAgent.agentCode}" class="form-control" id="toAgent">
								<label>转让人编号</label>
								<input type="text" name="fromAgent" value="${parameter.entity.userByFromAgent.agentCode}" class="form-control" id="toAgent">
									...
									<tr data-row="${l.id}">
										...
										<c:if test="${user.admin}">
											<c:if test="${l.type.desc eq '失败/未付款'}">
												<th>
													<a class="btn btn-success default ajaxLink" href="<c:url value='/advance/pay/dmz/aliNotify.html?orderId=${l.id}&&money=${l.voucher}' /> ">人工充值</a>
													<a class="btn btn-success default ajaxLink" href="<c:url value='/voucher/remove.json?id=${l.id}' /> ">删除纪录</a>
												</th>
											</c:if>
