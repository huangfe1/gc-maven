		...
		<div class="modal-body">
			<div class="container-fluid">				
					...
					<ul class="nav nav-tabs" role="tablist">
						 ...
						 <li role="presentation"><a href="#authedInfo" aria-controls="authedInfo" role="tab" data-toggle="tab">库存信息</a></li>
						 <li role="presentation"><a href="#accountsInfo" aria-controls="accountsInfo" role="tab" data-toggle="tab">账户信息</a></li>
					...
					<div class="tab-content">
    					...
    					<div class="col-md-12 col-xs-12">
    						<div class="form-group">
									...
									<div class="form-group">
										...
										<%--</div>--%>
										<label for="currentPoint" class="col-sm-2 control-label">手机号</label>
										<div class="col-sm-4">
    					...
    					<div role="tabpanel" class="tab-pane" id="accountsInfo">
    					<div class="col-md-12">
								<table class="table table-striped table-bordered table-condensed">
									<thead>
										<tr>
									...
									<c:forEach items="${aTypes}" var="type">
											<tr>
												<td>${type.stateInfo}</td>
												<td>${parameter.entity.accounts.getAccount(type)}</td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
							...
							<div class="col-md-12">
								<table class="table table-striped table-bordered table-condensed">
									...
									<tbody id="agentStatus" >
										<tr>
											<td><select class="form-control agentStatus" <c:if test="${user.agent}"> readonly="readonly" disabled="disabled"</c:if>
														  data-goods="${parameter.entity.id}">
												<c:forEach items="${agentStatus}" var="status">
													<option value="${status.state}"
															<c:if test="${parameter.entity.agentStatus.state  eq status.state}">
																selected="selected" </c:if>>${status.desc}</option>
												</c:forEach>
											</select></td>
										</tr>
									</tbody>
...
<script type="text/javascript">
	...
	function init() {
        ...
        $("#agentStatus").delegate(".agentStatus","change",function(e){
            var that=$(this);
            var param={
                "id":that.attr("data-goods"),
                "tid":that.val(),
            }
            $.ajax({method:"POST",data:param,url:"<c:url value='/agent/changeStatus.json'/>",success:function(){
                alert("修改成功");
            },error:function(){alert("修改失败");}})
        })
