...
<body>
	...
	<c:if test="${ empty shopcart or shopcart.quantity<1}">
...
<div class="container">
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-12 col-xs-12">
			<a class="btn btn-warning btn-lg btn-block" id="btn-calcuate" href="#"  <c:if test="${ empty shopcart or shopcart.quantity<1}">disabled='disabled'</c:if> >
		...
		<br>
			...
			<div class="row">
			<div class="col-md-12 col-xs-12">
			<a style="display: none" class="btn btn-warning btn-lg btn-block" id="btn-transfer" href="#"  <c:if test="${ empty shopcart or shopcart.quantity<1}">disabled='disabled'</c:if> >
		...
		<br>
				...
				<div class="row">
			<div class="col-md-12 col-xs-12">
				<a style="display: none" class="btn btn-warning btn-lg btn-block" href="<c:url value='/vmall/shopcart/pay.html?ref=${agentCode}'/>" id="payBtn" <c:if test="${ empty shopcart or shopcart.quantity<1}">disabled='disabled'</c:if> >
		...
		<div class="row">
			<div class="col-md-12 col-xs-12">
				<a  class="btn btn-info btn-lg btn-block" href="<c:url value='/dmz/vmall/index.html?ref=${agentCode}'/>">
	...
	<script type="text/javascript">
		$(function() {
			...
			$("#btn-calcuate").click(function(){
				...
				<%--$.ajax({--%>
					<%--type: "POST",--%>
					<%--url: "<c:url value='/transfer/hf_calculate.json'/>",--%>
					<%--cache: false,--%>
				...
				$.ajax("<c:url value='/transfer/hf_calculate.json'/>",
...
//							}
							if(m.flag=="0"){
								//弹出审核窗口
								$("#winModal").load("<c:url value='/transfer/calculate.html?type=mall'/>",function(e) {
									$('#winModal').modal({
										backdrop : "static"
									});
								});
					...
					$("#btn-transfer").click(function(){
						 ...
						 if(str==null||str==""){
							 alert("汇款信息必填");
							 		...
							 		var useVoucher=$("#useVoucher").is(":checked")?1:0;//选中为1  没有选中为0
									$.ajax("<c:url value='/transfer/hf_applyFrom.json'/>",
											{"type":"POST","data":{"remittance":str,"useVoucher":useVoucher},"success":function(xhr){
												var  m = xhr;
												if(m.flag=="0"){
												alert("申请成功！");
											
												  location.href="<c:url value='/dmz/vmall/index.html?ref=${agentCode}'/>";
												}else{
			...
			$("#cartitems").delegate(".quantity","change",function(e){
				...
				$.ajax("<c:url value='/vmall/shopcart/put.json'/>",{"type":"POST","data":{"goodsId":goodsId,"quantity":val},"success":function(xhr){
			...
			function removeItem($this,goodsId){
				$.ajax("<c:url value='/vmall/shopcart/remove.json'/>",
