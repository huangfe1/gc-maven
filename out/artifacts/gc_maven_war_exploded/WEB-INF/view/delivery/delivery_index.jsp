            ...
            <div class="container-fluid">
                ...
                <form class="navbar-form navbar-left" id="searchForm"
                    ...
                    <div class="form-group">
                        ...
                        <%--value="${consigneeName}" name="entity.consigneeName" id="consigneeName"--%>
                        <%--autofocus class="typeahead form-control" placeholder="收货人姓名">--%>
                        <label class="">申请发货人</label> <input type="text"
                                                             value="${parameter.entity.applyAgent.agentCode}"
                                                             name="entity.applyAgent.agentCode" id="applyAgent"
                                                             autofocus class="typeahead form-control"
                                                             placeholder="申请发货人编码">
                        <label class="">联系电话</label> <input type="text"
                                                            value="${parameter.entity.address.mobile}"
                                                            name="entity.address.mobile" id="mobile"
                                                            autofocus class="typeahead form-control" placeholder="电话">

                        <label class="">收货人</label> <input
                            value="${parameter.entity.address.consignee}" name="entity.address.consignee" id="consignee"
                            autofocus class="typeahead form-control" placeholder="">
                        ...
                        <select name="limit" id="limit" class="form-control">
                            <option value="10000000">不限</option>
                    ...
                    <table
                        ...
                        <c:forEach items="${dls}" var="g">
                            <tr data-row="${g.id}">
                                ...
                                <td>${g.orderNo}</td>
                                <td>${g.address.consignee}</td>
                                <td>${g.applyAgent.realName}</td>
                                <td>${g.applyAgent.agentCode}</td>
                                <td>${g.quantity}</td>
                                <td>${g.address.mobile}</td>
                                ...
                                <td>
                                    ...
                                    <c:if test="${g.paymentStatus=='UNPAID'}">
                                        <a class="btn btn-danger default editBtn" href="<c:url value='/delivery/delivery.html?id=${g.id}' /> "><span class="glyphicon glyphicon-ok fa-fw" aria-hidden="true"></span>确认付款</a>
                                    </c:if>

                                    <c:if test="${g.paymentStatus=='COD'}">
                                        <a class="btn btn-danger default ajaxLink" href="<c:url value='/mobile/note/confirmPay.json?id=${g.id}' /> "><span class="glyphicon glyphicon-ok fa-fw" aria-hidden="true"></span>确认付款</a>
                                    </c:if>

                                    <c:if test="${g.status=='DELIVERY' }">
                                        <a class="btn btn-info default editBtn" href="<c:url value='/delivery/delivery.html?id=${g.id}' /> "><span class="glyphicon glyphicon-wrench fa-fw" aria-hidden="true"></span>修改物流</a>
                                        <a class="btn btn-info  printBtn" href="<c:url value='/delivery/print.html?id=${g.id}' /> " target="print"><span class="glyphicon glyphicon-print fa-fw" aria-hidden="true"></span>打印</a>
                                    </c:if>
                                        <%--</c:when>--%>
                                        <%--</c:choose>	--%>
                                    <a class="btn btn-danger default ajaxLink" data-role="delete" <c:if test="${g.status!='NEW' }">disabled="disabled"</c:if> href="<c:url value='/delivery/remove.json?id=${g.id}' /> "><span class="fa fa-trash fa-fw" aria-hidden="true"></span>删除</a>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#downNew").click(function (e) {
            ...
            e.preventDefault();
            var limit = $("#limit").val();
            var url = "<c:url value='/delivery/getOrders.html'/>" + "?limit=" + limit;
//                console.log(url);
                ...
                url: $(e.target).attr("href"), method: method, complete: function (xhr, ts) {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        if (m.flag == "0") {
                            ...
                            alert($("#messageBody").html() + "0" + m.message);
                            $("#editModal").modal({backdrop: "static", show: true});
                        }
